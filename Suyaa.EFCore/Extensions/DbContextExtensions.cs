...
namespace Suyaa.EFCore
{
    ...
    public static class DbContextExtensions
    {
        ...
        private const string TYPE_NAME_ENTITY = "Suyaa.EFCore.Dbsets.Entity`1";
        ...
        public static IDatabaseConnectionInfo GetDatabaseConnectionInfo(this DbContext context)
        {
            ...
            return type switch
            {
                var h when h == DatabaseTypes.PostgreSQL => (IDatabaseConnectionInfo)Activator.CreateInstance(Type.GetType("Suyaa.Data.PostgreSQL.NpgsqlConnectionInfo")),
                _ => throw new Exception($"不支持的数据库提供商'{context.Database.ProviderName}'")
            };
        }
